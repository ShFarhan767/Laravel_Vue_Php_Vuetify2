<template id="">
    <span>
        <div class="card">
            ...
            <form action="" method="post" @submit.prevent="sliderEntry()">
                    ...
                    <div class="supplier-entry py-4 px-2 comm-form-back-img">
                        <div
                            ...
                            <div class="form-group">
                                <label> LargeScreenImage <i class="fa fa-star text-danger text-10 cursor-pointer"
                                        title="Mendatory Field" data-placement="top" data-toggle="tooltip"
                                        data-original-title="Mendatory Field"></i></label>
                                <input type="text" v-model="form.largeScreenImage" class="form-control"
                                    placeholder="Enter LargeScreenImage"
                                    :class="{ 'is-invalid': form.errors.has('largeScreenImage') }">
                                <has-error :form="form" field="largeScreenImage"></has-error>
                            </div>



                            <div class="form-group">
                                <label> SmallScreenImage <i class="fa fa-star text-danger text-10 cursor-pointer"
                                        title="Mendatory Field" data-placement="top" data-toggle="tooltip"
                                        data-original-title="Mendatory Field"></i></label>
                                <input type="text" v-model="form.smallScreenImage" class="form-control"
                                    placeholder="Enter SmallScreenImage"
                                    :class="{ 'is-invalid': form.errors.has('smallScreenImage') }">
                                <has-error :form="form" field="smallScreenImage"></has-error>
                            </div>
                            ...
                            <div class="form-group">
                                <label> ButtonText <i class="fa fa-star text-danger text-10 cursor-pointer"
                                        title="Mendatory Field" data-placement="top" data-toggle="tooltip"
                                        data-original-title="Mendatory Field"></i></label>
                                <input type="text" v-model="form.buttonText" class="form-control"
                                    placeholder="Enter ButtonText"
                                    :class="{ 'is-invalid': form.errors.has('buttonText') }">
                                <has-error :form="form" field="buttonText"></has-error>
                            </div>



                            <div class="form-group">
                                <label> ButtonLink <i class="fa fa-star text-danger text-10 cursor-pointer"
                                        title="Mendatory Field" data-placement="top" data-toggle="tooltip"
                                        data-original-title="Mendatory Field"></i></label>
                                <input type="text" v-model="form.buttonLink" class="form-control"
                                    placeholder="Enter ButtonLink"
                                    :class="{ 'is-invalid': form.errors.has('buttonLink') }">
                                <has-error :form="form" field="buttonLink"></has-error>
                            </div>
                ...
                <table style="width: 100%;" id="example" class="table table-hover table-striped table-bordered">
                    <thead>
                        <tr style="background:rgba(242, 242, 242, 0.47)">
                            <th>SN</th>
                            <th>LargeScreenImage</th>
                            <th>SmallScreenImage</th>
                            <th>Title</th>
                            <th>Description</th>
                            <th>ButtonText</th>
                            <th>ButtonLink</th>
                    ...
                    <tbody>
                        <tr v-for="(singleList, index) in sliderLists " :key="index">
                            <td>{{ index + 1 }}</td>
                            <td>{{ singleList.largeScreenImage }}</td>
                            <td>{{ singleList.smallScreenImage }}</td>
                            <td>{{ singleList.Title }}</td>
                            <td>{{ singleList.description }}</td>
                            <td>{{ singleList.buttonText }}</td>
                            <td>{{ singleList.buttonLink }}</td>
...
export default {
    ...
    methods: {
        ...
        sliderEntry() {
            ...
            this.form.post("slider").then((response) => {
        ...
        sliderShow() {
            ...
            this.form.get("/slider").then((res) => {
                this.sliderLists = res.data.data;
            });
