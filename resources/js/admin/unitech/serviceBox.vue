<template id="">
    <span>
        <div class="card">
            ...
            <form action="" method="post" @submit.prevent="serviceBoxEntry()">
                <div class="product-entry">
                    <div class="supplier-entry py-4 px-2 comm-form-back-img">
                        <div
                            ...
                            <div class="form-group">
                                <label> Icon <i class="fa fa-star text-danger text-10 cursor-pointer"
                                        title="Mendatory Field" data-placement="top" data-toggle="tooltip"
                                        data-original-title="Mendatory Field"></i></label>
                                <input type="text" v-model="form.Icon" class="form-control" placeholder="Enter Icon"
        ...
        <div class="supplier-entry py-1 px-1">
            <div class="card-body table-responsive bg-white">
                <table style="width: 100%;" id="example" class="table table-hover table-striped table-bordered">
                    ...
                    <tbody>
                        <tr v-for="(singleList, index) in serviceBoxLists " :key="index">
                            <td>{{ index + 1 }}</td>
                            <td>{{ singleList.Title }}</td>
                            <td>{{ singleList.Icon }}</td>
...
export default {
    ...
    methods: {
        ...
        serviceBoxEntry() {
            ...
            this.form.post("serviceBox").then((response) => {
        ...
        serviceBoxShow() {
            ...
            this.form.get("/serviceBox").then((res) => {
                this.serviceBoxLists = res.data.data;
            });

        },

        serviceBoxDelete($id) {
            axios.delete("/serviceBox/" + $id).then((res) => {
                Toast.fire({
                    icon: "success",
