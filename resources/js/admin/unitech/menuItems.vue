<template id="">
    <span>
        <div class="card">
            ...
            <form action="" method="post" @submit.prevent="menuItemsEntry()">
                    ...
                    <div class="supplier-entry py-4 px-2 comm-form-back-img">
                        <div
                            ...
                            <div class="form-group">
                                <label> MenuName <i class="fa fa-star text-danger text-10 cursor-pointer"
                                        title="Mendatory Field" data-placement="top" data-toggle="tooltip"
                                        data-original-title="Mendatory Field"></i></label>
                                <input type="text" v-model="form.menuName" class="form-control"
                                    placeholder="Enter MenuName" :class="{ 'is-invalid': form.errors.has('menuName') }">
                            ...
                            <div class="form-group">
                                <label> MenuLink <i class="fa fa-star text-danger text-10 cursor-pointer"
                                        title="Mendatory Field" data-placement="top" data-toggle="tooltip"
                                        data-original-title="Mendatory Field"></i></label>
                                <input type="text" v-model="form.menuLink" class="form-control"
                                    placeholder="Enter MenuLink" :class="{ 'is-invalid': form.errors.has('menuLink') }">
                                <has-error :form="form" field="menuLink"></has-error>
                            </div>
                            <!-- ---------- /input field -->
                        </div>
                    </div>
        ...
        <div class="supplier-entry py-1 px-1">
            <div class="card-body table-responsive bg-white">
                <table style="width: 100%;" id="example" class="table table-hover table-striped table-bordered">
                    ...
                    <tbody>
                        <tr v-for="(singleList, index) in menuItemsLists" :key="index">
                            <td>{{ index + 1 }}</td>
                            <td> {{ singleList.menuName }} </td>
                            <td> {{ singleList.menuLink }} </td>
                            <td>
...
export default {
    ...
    methods: {
        ...
        menuItemsEntry() {
            ...
            this.form.post("menuItems").then((response) => {
        ...
        menuItemsShow() {
            ...
            this.form.get("/menuItems").then((res) => {
                this.menuItemsLists = res.data.data;
            });

        },

        menuItemsDelete($id) {
            axios.delete("/menuItems/" + $id).then((res) => {
                Toast.fire({
                    icon: "success",
